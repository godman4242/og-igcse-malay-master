...
**START AT A11** (`src/hooks/useWritingEvaluator.js:202` — "Get AI feedback" burns one of the 50
daily AI calls and renders absolutely nothing when the reply isn't JSON: no feedback, no error, no
explanation, and the quota is spent). **A1–A10 + B4 are SHIPPED** — see the "BATCH A PROGRESS" table
below for what landed and the evidence; do NOT re-take them. **12 Batch A rows remain (~33 h):
A11–A22.**
