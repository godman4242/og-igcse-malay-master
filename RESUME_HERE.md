...
### → THE KICKOFF (copy everything between the `'''` lines): Malay On-Ramp Phase 2 — full 0546 list + syllabus topic packs
...
```
'''
Malay On-Ramp Phase 2 (content depth). Fold the full official-0546 vocabulary list into `src/data/dictionary.js` and expand `TOPIC_PACKS` (`src/data/topics.js`) to the A–E syllabus buckets. HIGH learning value (core mission) but a LARGE Malay-verification grind — Claude is the quality gate for ~1000+ words, so batch + web-verify EVERY gloss + IGCSE example before it ships (confident-wrong Malay is the worst defect). Needs its OWN research→design→build session (superpowers:brainstorming → writing-plans BEFORE any code), NOT a tail-of-session build.

⚡ ACTIVATE FIRST: Claude Code CLI in repo `og igcse malay master` · **Opus 4.8 @ xhigh + `/fast`** (or Fable 5 `high` for the long verification grind ONLY IF access re-confirmed — past the 2026-07-12 time-box) · start on `main` (pull first) → branch `feat/malay-onramp-p2` · **WebSearch ON** (verify every Malay gloss against real sources — PRPM / Kamus Dewan / the official 0546 list; do NOT rely on memory) · run **superpowers:brainstorming → writing-plans BEFORE any code**.

READ FIRST: `docs/loop/GOAL.md` → 🎖️ Kheshav-ranked epics #2 · the Phase-1 spec `docs/superpowers/specs/2026-07-14-malay-starter-deck-design.md` (§Non-goals scopes Phase 2) · `src/data/dictionary.js` + `src/data/topics.js` (current shape) · `src/data/malayStarter.js` (the shipped 45-word starter this extends).

PROVE IT (measurable DONE): every new gloss web-verified (paste sources for a sample) · canonical-list tests pin the new words · gate green (build + tests + lint) · no STORE_VERSION change unless a new lazy slice needs one · README + tour updated · RESUME_HERE + GOAL.md refreshed, this kickoff superseded.
'''
```
