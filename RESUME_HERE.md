...
> - **Then, while verifying signed in, found the ai-proxy tier DEAD for every learner** (pre-existing): its `ALLOWED_ORIGINS`
>   secret was never set (CORS-blocked on the live site) and all 4 hardcoded `:free` models were retired. Fixed live — secret
>   set, model list now ends with `openrouter/free`; chat + MS/EN roleplay return 200 signed in. Counters matched exactly.
> - **Money: nothing can be billed** — no Google Cloud billing account; both Gemini keys are free tier. **Signup stays open**
>   (it already is the invariant). Remaining follow-ups (legacy keys, client polish, free-model Malay slips): GOAL item 7 c–i.
...
> - **Headers:** CSP ENFORCED + `Permissions-Policy`. Caught before shipping: the service worker fetches Google Fonts under `connect-src`, so enforcing the old list would have broken fonts for returning visitors. `pdf.js` runs with `isEvalSupported:false`. Proven with 0 violations on local-with-real-headers, the Vercel preview, AND production: 23 routes, photo + scanned-PDF OCR, PDF + Layout view, Whisper transcription, cancel mid-run, BYOK Test-key, Google sign-in, offline reload.
> - **Guard hole:** `~/.claude/hooks/launch-gate-guard.py` now treats commit / cherry-pick / revert (post-commit) and merge / pull (post-merge) as deploys when that hook pushes. 17 tests in `~/.claude/hooks/test_launch_gate_guard.py` (11 watched failing first). Proven on this repo: commit DENIED while red, allowed while green.
> - **Fresh-context review: 10 findings, 9 fixed.** Guard: `git -C x push` / `--no-pager push`, escaped-space and `~`/`$HOME` paths, and a red repo hiding behind a green session repo all slipped through (now closed, tested). Gate: `script-src-attr 'unsafe-inline'` stopped warning (my regression), and a REVOKE before a table re-create still counted (agent-harness `4258a1a`). Privacy page: date, and Translate now says the TEXT goes to Google. Docs: `.env.example` / `SETUP_APIS.md` named secret keys `VITE_*` that no code reads. Tests: `vite preview` now serves `vercel.json`'s headers and `tests/e2e/csp.spec.js` fails on any CSP violation (red-proofed with 2 planted breaks). Skipped: a hook that pushes via another script (documented limit).
...
**START AT A12** — the streak freeze refunds itself. **A1–A11 + B4 are SHIPPED** — see the "BATCH A
PROGRESS" table below for what landed and the evidence; do NOT re-take them. **11 Batch A rows remain
(~29.5 h): A12–A22.**

A12, mechanism re-derived at HEAD on 2026-08-08 (the census's one-liner is right but its anchor is
stale): in `updateStreak` (`src/store/useStore.js`, currently ~line 1573), the missed-day branch
consumes a freeze (`streakFreezes -= 1`) but **does not increment `streak.count`**. Execution then
falls into `if (checkStreakMilestone(streak.count))`, which re-tests the *unchanged* count. Milestones
are `[7, 14, 30, 50, 100, 365]` (`src/lib/confetti.js:59`). So a learner sitting on exactly 7 who
misses a day gets the freeze **refunded (+1)** and the milestone confetti **re-fired** — net zero cost,
celebration repeats, and they can miss a day indefinitely. The loss-aversion mechanic stops meaning
anything. ⚠️ There is **no existing streak test file** (`updateStreak` is only touched incidentally by
...
A7's sweep turned up an un-swept C4 site. Budget for that, and add the sibling fix to the same commit.

Batch B's 7 decisions are MADE (Kheshav said "your pick" on 2026-08-06) and recorded in the census
doc's Batch B table — B4 shipped with A7; **B1, B2, B3, B5, B6, B7 remain (~14 h)** and can now be
built without asking. Highest value of those: **B2** (English SVA answers 12/12 at option B — a
student scores full marks tapping the second option without reading) and **B1** (the Grammar deck
serves only half its drills, then loops on those forever).
...
⚠️ ONE exception, flagged: **A16** (`addRoleplayHistory` / `addCikguMessage` / `addMistake` in
`useStore.js` never stamp `lastMutationAt`, so a signed-in reload restores stale cloud over them) rests
on an agent's word alone — it is the ONE confirmed finding the lead session did not re-verify by hand.
Read those three setters plus `AuthGuard.jsx:119-134` yourself before touching it; if it does not
reproduce, mark it REFUTED with the receipt and move on.

⛔ **Do NOT trust `PROBLEMS.md`** (untracked, auto-generated by the JClaw "Clawdia" agent). Audited
2026-08-08: of its 8 "Active Problems", **1 was real** (the authGuard flake, already tracked) — 3 are
the documented accepted lint warnings, 2 are cosmetic build warnings, 1 proposes a fix that is already
implemented (`transformers` IS already `await import()`-ed in `transcribeEngine.js`), and 1 chases a
`--localstorage-file` flag that is not in this repo's config at all. Its headline "Suggested fix" —
raise the authGuard timeout — was **measured and disproven** on 2026-08-03.
...
verified against PRPM / Kamus Dewan, never memory — fetchable at
`https://prpm.dbp.gov.my/cari1?keyword=<word>`. No *remaining* Batch A row is a Malay-content item, so
you may not need it — but keep it on, because sweeps keep turning content defects up next to code ones).
...
|---|---|---|---|
| A11 | "Get AI feedback" spent a daily call and showed nothing | ✅ shipped | `tryParseJSON` returns **null** for a reply it can't read (prose instead of JSON, or empty), and the v1 path did `setAiFeedback(parsed)` — so null was indistinguishable from "never asked": spinner runs, screen returns to blank, one of the 50 daily AI calls gone. Fixed by giving v1 the signal the **v2 path already had** (`v2ParseRejected` → "Annotated feedback unavailable…") rather than inventing a mechanism: new `aiFeedbackRejected`, cleared on retry and on `reset()`, surfaced as a `role="status"` notice that says the call was spent and the written analysis is unaffected. A thrown request sets it too — the quota is gone either way. 6 tests, 4 red first; the readable-JSON happy path passes in both states so it can't go vacuous. |
| A10 | Flashcard leaked its answer to screen readers | ✅ shipped | **Two leaks; the census only found one.** (1) `backface-hidden` + `rotate-y-180` are VISUAL CSS — they don't remove a face from the accessibility tree, so VoiceOver read straight through and announced "rumah … house … Saya tinggal di rumah" in one pass. Fixed with `inert` on whichever face is turned away (not just `aria-hidden`, because the front face holds real buttons that must leave the focus order too — same precedent as `Layout.jsx`'s theater-mode fix, P1-01). (2) **Found by sweeping, and worse:** the FRONT face passed `meaning={card.e}` to `DictionaryIcon`, which builds its accessible name as `` `${word} — ${meaning}` `` — so the gloss was announced *before any flip at all*. Every other `DictionaryIcon` call site is post-reveal and keeps its meaning; the flashcard front is the only pre-reveal one, so the fix is at the call site. Red-proofed by reverting only that prop: the accessible name went back to `'rumah — house'`. ⚠️ Test gotcha: jsdom's `textContent` reports an `inert` subtree like any other, so a whole-container assertion can never tell a fixed card from a broken one — scope it to the front face. |
| A9 | Dashboard Quick Review was mouse-only | ✅ shipped | Bare `div onClick` with no role/tabIndex/key handler — and because the Again/Good/Easy buttons only render once flipped, a keyboard or switch user couldn't reveal the answer **or** reach the ratings, so the whole "study without leaving the Dashboard" affordance was unusable end to end. Now `role="button"` + `tabIndex={0}` + `aria-expanded` + Enter **and** Space (a role=button must answer to both; Space also `preventDefault`s so it doesn't scroll the page). Kept as a div, not a real `<button>`, because it contains the pronounce button and nesting buttons is invalid HTML. Also bumped that pronounce control from **20×20 → 44×44** with an `aria-label` — once the card is focusable, a 20px nested target is a worse trap than before. 6 tests, 5 red first. |
| A8 | A failed cloud push was invisible — pill still said "Synced" | ✅ shipped | `triggerCloudSync`'s catch was a bare `console.warn`, so an expired token or a **paused free-tier Supabase project** (a documented recurring event here) left a signed-in learner believing their work was backed up while everything since existed only on that device. The fix reuses the mechanism C1-hardening already built rather than inventing one: `setCloudUnavailable` raises `sync.cloudUnavailable` → red "Cloud backup unavailable" pill with Retry. Two other sites already call it; this one was never wired. Success path now clears it too. 3 tests, 2 red first — **no mocking**: `pushStateBlob` throws "Supabase not configured" without a client, so the test drives the real failure path. ⚠️ Gotcha for the next person: `triggerCloudSync` does `await import()` inside a 5 s debounce, and under fake timers that first-time module resolution doesn't settle within the flush — warm the module cache with a top-level `await import('../../config/supabase')` or the test silently reads as "no failure happened". |
| A7 + **B4** | `gtx` echoed the source word as a *successful* translation; the reader minted it as a card | ✅ shipped | Two halves of one confident-wrong path. **A7:** `gtx.js` did `data?.[0]?.map(…).join('') \|\| text` and returned `source: 'gtx'` — so a 200 with an empty/unexpected body returned the SOURCE WORD tagged as success, bypassing every `source === 'error'` guard, including `writeCache`'s, whose comment describes this exact hazard ("usually the input word AS its own translation… permanently serving the word as its own gloss"). Now returns `source: 'error'` when nothing parsed — checking that we parsed nothing, **not** that output equals input, so `radio → radio` still succeeds. **B4** (your "your pick" → *block the add and say why*): `PDFReader.addSelectionToDeck` did `translations[pi++]?.text \|\| s.word`, ignoring `source`. New pure `partitionSelectionForDeck` holds failures back, leaves them selected so retry is one tap, and a `role="status"` notice explains it. Red-proof = running the OLD logic on the same inputs: `makan=makan`, `makarn=makarn`, `rumah=house \| xyzq=xyzq`. **Sweep also caught a C4 sibling site nobody had swept:** `ex: \`${'{'}s.word{'}'} — ${'{'}s.en{'}'}\`` is a fabricated example with no parentheses, so it slips past `speakTarget`'s `PLACEHOLDER_EX` and Speak mode would read the English gloss aloud in a Malay voice. Now `ex: ''`, matching C4's fix. |
| A6 | Manual "+ Card" hard-gated to `language === 'ms'` | ✅ shipped | `MistakeJournal.jsx:257` required `m.language === 'ms'`, so a 0510 English learner could never hand-add an English miss — even one carrying both a word and a correction. A pre-v34 leftover: the store has been bilingual since (`promoteMistakeToCard` derives the card's target language from `mistake.language`), so nothing below the UI changed. Manual promotion is a deliberate tap, so unlike the AUTO gate (`canAutoPromoteMistake`, ms=vocab+imbuhan / en=vocab-only) it is **not** category-gated. Red-proofed by reverting the fix: the Malay fixture guard passed in BOTH states while the English cases failed, so the test can't pass vacuously. |
| A5 | `grammarStats` missing from `BACKUP_KEYS` | ✅ shipped | Persisted + rendered on the Dashboard but absent from `makeBackupDefaults()`, so a backup restored on a new phone kept the grammar SCHEDULE (`grammarCards`) and reset the SCORES to "0 drills · 0%" — reads as data loss. `src/store/CLAUDE.md` lists the fields that must NOT cross devices and `grammarStats` isn't one, confirming an omission not a choice. **Second time this list has drifted** (P2-C6 was the first), so it ships with a **drift guard** asserting every persisted field is either backed up or on the documented device-local list — it found exactly 2 items, `grammarStats` (the defect) and `_version` (zustand's own schema marker, correctly local). |
| A3+A4 | Comeback warm-up skipped, repeated, and could hard-block Study | ✅ shipped `bf594a6` | See the commit. Root cause was TWO orderings behind ONE index; replaced with one queue via pure `buildSessionQueue` (`src/lib/studyQueue.js`). Measured bad sequence before the fix: `kata0 kata1 kata2 kata3 kata4 kata0 kata1 kata2` — it *re-ran* the warm-up rather than merely skipping ahead, which is worse than the census claimed. 5 hook-level tests, 4 watched failing first. ⚠️ This commit landed **directly on main** (a `;` vs `&&` slip in a chained shell command meant the merge step ran after an aborted commit) — gate was green, so it is safe, just not on its own branch. |
| A2 | GOAL.md pointed the build loop at shipped work | ✅ shipped | Struck C4/C5/C6/C7/C8/C9 (commit `1464049` closes that queue; each re-confirmed against live code, incl. all four of C9's drift claims now correct in CLAUDE.md) and retired the "then work the 🟡 queue highest-severity-first" instruction. ⚠️ **And a correction of my own:** I first marked `0-quater` (authGuard flake) RESOLVED on the strength of RESUME_HERE's "4 clean runs" — then it **failed in a real pre-commit gate ~60 s after a clean 242-file/2345-test run**. Reopened with that evidence; `af7997d` lowered the rate but did not eliminate it. |
...
| C1-hardening | A dead/paused backend failed silently | ✅ shipped | `enabled` **stays** a presence check (12 render-path consumers; async there = a network hop in first paint) — instead reachability is reported at the point of failure. `hydrateCloudData` + AuthGuard's blob catch (the two calls a paused project trips, previously a `console.warn` and a telemetry event *sent to the dead backend*) now call `setCloudUnavailable`, raising `sync.cloudUnavailable`; the header pill reads **"Cloud backup unavailable"** in red with Retry ENABLED (it was `disabled` whenever the queue was empty — and an unreachable backend usually leaves an empty queue, which is exactly how the pause hid). Flag is session-scoped: `reconcileSyncStatusOnLoad` clears it so it can't lie on a cold load. Pill text extracted to pure `cloudPillLabel` so the visible-error contract is testable without mounting Layout. |
| C4 | Fabricated `card.ex` spoken aloud in Speak mode | ✅ shipped | **The review named 1 site; sweeping every `ex:` in a card payload found 4.** All defeat `PLACEHOLDER_EX`: `SharedDeckImport` (`rumah — house`), `RoleplayScorecard` (`From roleplay scenario: At the Restaurant` — English meta read in ms-MY), `WordFamilyTree` (`mengajar (verb) — from root "ajar"` — has parens, but the trailing text breaks the anchored regex), `AIFeedbackPanel` (`men- + t → t drops` — a grammar rule). SharedDeckImport mirrors `SearchModal` (real curated example when the dictionary has one, else the guard-recognised placeholder) — better than the review's `ex: ''` because `sanitiseDeck` whitelists only `{m,e,t,lang}` so no real example can ever transport. The other three get `ex: ''`; `card.mn` has **no consumer**, so relocating the metadata there would be a dead write. Entry chunk unmoved (527.14 → 527.15 KB — `dictionaryExamples` was already eager via SearchModal). |
...
+ the touched-area e2e spec green. Full suite after the queue: **229 files / 2251 tests**, CI green,
all 12 prod deploys READY.
...
#### 📊 What remains from the 2026-08-01 review (the whole landscape, as of 2026-08-03)
...
So the honest state: the review's verified work is nearly done, and the large remaining body (120 items)
is *unverified claims*, not a fix queue. Verifying them is its own project, not a batch to grind through.

### → QUEUED (after the C-queue above): Dictionary examples — continue Batch 11+ (coverage 704 → 825)
...
1. ~~`['lisense','license']`~~ — **RESOLVED 2026-08-03.** British English splits *licence* (noun) /
   *license* (verb). Pointed at the NOUN, `licence`: it dominates in IGCSE writing, and the app's own
   content already teaches it three times (`dictionary.js:635` 'lesen memandu' → 'driving licence',
   `dictionaryEn.js:153`, `academicEn.js:56`). Left alone on purpose: `wikidataMalayEn.js:2160`
   `{ m:'lesen', e:'license' }` — that file is a committed snapshot of external Wikidata (CC0), used
   for AI-pair GROUNDING not teaching, and is regenerated by a SPARQL pull, so a hand-edit would be
   both wrong and lost.
...
> ⛔ **CORRECTED 2026-08-05 — the `991` clause above is WRONG.** APM's 991 was folded into 999 on
> 1 Oct 2007 (`civildefence.gov.my`), so the passage taught a defunct number. Fixed by `e51e394`
> (V1): passage + `correctIndex` + explanation all now `999`, pinned by `listeningPassages.test.js`.
> "Internally consistent" was true; "the real line" was not — consistency is not content truth.
