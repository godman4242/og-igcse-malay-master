...
### → THE KICKOFF: none pasted — the next bet is UNCHOSEN. Kheshav picks from the candidates below.
...
**Candidates flagged this session (decide-and-flag; Kheshav chooses the next bet):**
- **SEO + a11y hardening workstream** — from `~/Downloads/igcse-malay-master-audit.md` (JClaw audit, 2026-07-14). Real issues: per-page canonical/title/description all point to homepage `/` (Google sees every page as a dupe); the OG deployment's sitemap/robots/OG-tags all point at UPG (OG invisible to search); one WCAG contrast fail on the FirstRunCard `--color-accent`(#ff4d6d)/`#fff` button (3.21:1). ⚠️ **Verify each finding against live code first** — the app already HAS a client-side `<Meta>` component (the real gap is crawler-facing = SSR/prerender, a bigger architectural call, not "add react-helmet") and already lazy-loads every non-Dashboard route (the audit's "42% unused JS / code-split routes" item is stale). Its own epic; log to GOAL.md.
- **Agentic-OS imagination session** — Kheshav wants to brainstorm what a personal agentic OS *could* do for 3 named pains (losing track of in-flight agents/projects · context resets each session · one cross-repo entry point). Framing: imagination-first, "I don't know what's possible." Parked this session; Kheshav wants it **on Fable**. ⚠️ **Fable access is PAST its 2026-07-12 time-box — re-confirm access before routing to Fable** ([[reference_fable5_vs_opus48_working]]).
- **Malay On-Ramp Phase 2** — fold the full official-0546 list into `dictionary.js` + expand `TOPIC_PACKS` (`src/data/topics.js`) to the A–E syllabus buckets, verified in batches (Phase 1 spec §Non-goals).
