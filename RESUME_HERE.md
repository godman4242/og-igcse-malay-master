...
WHY: ALL 16 CONFIRMED defects (#1–#16, incl. the P0 Malay grader and the #15 share-deck build) are SHIPPED gate-green (see the progress block below). What's left from the review is the 🟡 PLAUSIBLE list — findings extracted but NOT yet independently verified. The review's own instruction: "do not fix blind; verify first." **12 PLAUSIBLE items cleared** (VERIFIED REAL + FIXED): QuickReview studyLang leak · cloud `card_key` lang collision · `writingGrader` "sehinggakan" reward/flag contradiction · `pronunciation` positional cascade · `promoteMistakeToCard` cross-language link · `addMistake` dedupe-bump keeping `reviewed:true` · `PDFReader` in-flight sentence translation attaching doc A's English to doc B (document-epoch guard, red→green e2e) · the entire AI-plumbing cluster (`ai.js` non-stream double-encode + `ai.js` quota-masks-success + `gemini.js` abort/timeout disarmed-at-headers) · **the sign-in-merge data-safety cluster (2026-07-08): `useStore.hydrateCloudData` stale-local-card overwrite (freshness reconciliation by `(m,t,lang)` + `last_review`/`reps`) + `AuthGuard.handleSignIn` 0-card blob-restore skip (dropped the `cloudCardCount > 0` guard)** — one commit, red→green cross-device + jsdom tests. Plus a gate blocker fixed en route: a `learnerProfile`/`competenceSnapshot` wall-clock time-bomb that tripped the pre-commit gate today (own commit, first). **Remaining PLAUSIBLE queue (still unverified — reproduce first): Sync/store** (`syncEngine.processSyncQueue` retry reorders dependent add/remove + ~30s budget dead-letters deletions — both ANALYZED 2026-07-08 as likely-real-but-NARROW; fix DEFERRED, needs a queue-semantics design decision, NOT a blind patch: head-of-line-blocking on the reorder, dead-letter REPLAY on the lost deletion — see the review doc) **and Reader/multimodal** (`PDFReader` :317 index-keyed selection leak on doc replace · :926 cancel-retranslate race · :449 OCR worker leak · :411 unmount-while-recording leak · :529 engine failure misreported as "no clear speech" · `pdf.js:101` PDFDocumentProxy never destroyed). The AI + sign-in-merge clusters are now fully cleared. **E2E env note (SOLVED 2026-07-07):** the local app is NOT broken — the earlier "blank app under Playwright" was another project's dev server (`iaido-duel`) squatting on port 5173, reused by Playwright's `reuseExistingServer`. **If e2e ever looks broken again, run `lsof -i :5173` FIRST** — if it's serving the wrong app, `kill` it (or give this app its own port). With 5173 free, `guide-dictation` + the reader specs pass.
...
DO (per finding, TDD): reproduce it in a red test FIRST (prove it's real against live code) → if real, minimal fix → gate green, one cluster per commit; if NOT real, note "refuted — why" in the review doc and move on. Lead with the **Reader/multimodal** cluster (`PDFReader` :317/:411/:449/:529/:926 + `pdf.js:101` — the last unverified group; Sync/store is cleared except the two DEFERRED syncEngine design-calls). Run the touched-area e2e before shipping any UI change (GOAL #8) — the harness works now (mind port 5173).
