...
WHY: ALL 16 CONFIRMED defects (#1–#16, incl. the P0 Malay grader and the #15 share-deck build) are SHIPPED gate-green (see the progress block below). What's left from the review is the 🟡 PLAUSIBLE list — findings extracted but NOT yet independently verified. The review's own instruction: "do not fix blind; verify first." **7 PLAUSIBLE items cleared** (VERIFIED REAL + FIXED): QuickReview studyLang leak · cloud `card_key` lang collision · `writingGrader` "sehinggakan" reward/flag contradiction · `pronunciation` positional cascade · `promoteMistakeToCard` cross-language link · `addMistake` dedupe-bump keeping `reviewed:true` · `PDFReader` in-flight sentence translation attaching doc A's English to doc B (document-epoch guard, red→green e2e). **Remaining axis-1-ish candidate: `ai.js:196` double-encoded non-stream response** (lead with it), then the rest of the Sync/store · Reader/multimodal · AI · Perf/UX clusters (still unverified — reproduce first). **E2E env note (SOLVED 2026-07-07):** the local app is NOT broken — the earlier "blank app under Playwright" was another project's dev server (`iaido-duel`) squatting on port 5173, reused by Playwright's `reuseExistingServer`. **If e2e ever looks broken again, run `lsof -i :5173` FIRST** — if it's serving the wrong app, `kill` it (or give this app its own port). With 5173 free, `guide-dictation` + the reader specs pass.
