...
- ⏭ Next in queue: #8–9 dailyPlan (latest-band `.at(-1)` + non-today challenge guard) → #10–11 store/sync (cross-device tests mandatory) → #12 session-finish lang scope → #13 translate cache → #14 SSE buffer → #16 audio cap. (#15 share-link needs Kheshav's build-vs-remove decision.)
