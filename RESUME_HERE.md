...
- ⏭ Next in queue: **#14 SSE buffer** (`ai.js` — cross-chunk line buffer so a split `data:` line doesn't append raw JSON to the reply / drop the tail) → **#16 audio cap** (`transcribe.js` — enforce `MAX_AUDIO_SECONDS` after decode via `samples.length / sampleRate`). Then **#15 share-deck — Kheshav APPROVED BUILD (2026-07-06)**: consume `?deck=` → MakeDeckPanel-style import preview (see the share-deck build notes below).
