...
### → THE KICKOFF (copy everything between the `'''` lines): Land the Tutor Output Contract v1 (verify → smoke → merge), then refresh this handoff

*Superseded the "Malay Beginner On-Ramp Phase 1" kickoff on 2026-07-11. The On-Ramp bet is UNCHANGED — preserved as this kickoff's ALTERNATIVE and in full detail in the "✅ NEXT BET CHOSEN (2026-07-06)" bullet below. Reason for supersession: a finished v1 feature (`feat/tutor-output-contract`, 10 commits, flag-OFF, unmerged) was built AFTER the On-Ramp kickoff and left stranded; you don't start new work on top of an unmerged finished branch — land it first.*

```
'''
Verify, smoke-test, and LAND the Tutor Output Contract v1, then refresh this RESUME_HERE kickoff. The feature is BUILT on branch `feat/tutor-output-contract` (10 commits ahead of main, clean tree, unmerged) behind `TUTOR_CONTRACT_ENABLED` = false (inert until flipped).

⚡ ACTIVATE FIRST: Claude Code CLI in repo `og igcse malay master` · Opus 4.8 @ xhigh, /fast on (this is verify+land, not from-scratch) · check out branch `feat/tutor-output-contract` · Vercel MCP on (deploy check only) · no new skills/plugins/installs.

WHY: A finished v1 is stranded on a branch and this kickoff previously pointed at the OLDER "Malay Beginner On-Ramp" bet (never updated after the contract was built). The contract makes Cikgu Maya's AI-tier answers short-where-appropriate, one-next-action, and non-answer-dumping in retrieval contexts. Landing it (dormant is safe — flag OFF = zero behaviour change) clears the branch and unblocks the next bet.

READ FIRST: docs/superpowers/plans/2026-07-08-tutor-output-contract.md (§Deferred + §Self-Review = the flag-flip criteria: "eval conformance ≥95% + known leak recall + manual smoke") · docs/superpowers/specs/2026-07-08-tutor-output-contract-design.md · src/lib/tutorContract.js (the pure enforcer) · src/pages/CikguBot.jsx sendMessage (the flag-gated wiring; expert-tier branch exempt).

DO: (1) VERIFY green — `npm run build && npm run test:run && npm run lint` (paste output, don't assert) + `node scripts/ai-tier-eval/leakEval.mjs` → record precision/recall. (2) SMOKE — locally set `TUTOR_CONTRACT_ENABLED=true`, `npm run dev`: normal Cikgu question (answer intact), paste a 400-word essay (truncates at a sentence), voice mode (no ⟦CTRL⟧ tail read aloud), dark+light; then REVERT the flag to false. (3) DECIDE-AND-FLAG the flip: bring Kheshav the eval numbers + smoke result; recommend flip-ON only if ≥95% conformance + clean smoke, else land dormant. (4) LAND — merge `feat/tutor-output-contract` → main (or open a PR for a diff review first); confirm Vercel READY on upg-. (5) FIX RESUME_HERE — supersede this block: record tutor-contract SHIPPED (on/dormant) + write the next kickoff (default next bet = the On-Ramp below).

DON'T BREAK: flag stays default-OFF unless the eval+smoke justify ON (flipping ON changes Cikgu's LIVE answers — a quality call that needs the numbers first). Expert-tier Cikgu branch stays exempt from the enforcer. Prompt parity: any CIKGU_SYSTEM_PROMPT edit must mirror into scripts/ai-tier-eval/prompts.mjs (guarded by cikguSystemPrompt.test.js — "change one, change all"). No STORE_VERSION bump (v1 stores no new state).

DONE: gate green (pasted), leak precision/recall recorded, smoke done, flag decision made + flagged to Kheshav, branch on main, Vercel READY, this kickoff superseded with a fresh one.

ALTERNATIVE / default next bet after landing — "Malay Beginner On-Ramp Phase 1": design+build a curated, opt-in Malay survival-starter deck — `seedMalayStarter()` mirroring `seedEnglishStarter` (useStore.js ~:1315); ~40–50 highest-value words (greetings/pronouns/numbers/essential verbs+nouns/question words), each with an example sentence; opt-in one-tap on the empty-state; NEVER auto-seed; Malay/English decks never mix via cardsForLang/studyLang; every Malay entry Claude-verified (already caught pemandu="driver" mis-glossed "guide"). Full design in the "✅ NEXT BET CHOSEN (2026-07-06)" bullet below. Open with superpowers:brainstorming → writing-plans.
'''
```
