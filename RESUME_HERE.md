...
WHY: ALL 16 CONFIRMED defects (#1–#16, incl. the P0 Malay grader and the #15 share-deck build) are SHIPPED gate-green (see the progress block below). What's left from the review is the 🟡 PLAUSIBLE list — findings extracted but NOT yet independently verified. The review's own instruction: "do not fix blind; verify first." **6 PLAUSIBLE items cleared** (VERIFIED REAL + FIXED): QuickReview studyLang leak · cloud `card_key` lang collision · `writingGrader` "sehinggakan" reward/flag contradiction · `pronunciation` positional cascade · `promoteMistakeToCard` cross-language link · `addMistake` dedupe-bump keeping `reviewed:true`. **1 item VERIFIED REAL but ship-DEFERRED** — `PDFReader` in-flight sentence translation attaching doc A's English to doc B: root-cause fully traced + fix ready (document-epoch guard) + e2e written (`pdf-sentence-docswap.spec.js`, `test.fixme`), but **can't be verified because the local dev/e2e env renders a BLANK app under Playwright** (see ⚠️ below). **⚠️ DO THIS FIRST NEXT SESSION: fix the e2e env** — the local app is blank in Playwright Chromium (`#root` empty; EVERY store-binding spec fails: `pdf-replace`, `guide-dictation`, all; likely the Console Ninja VS Code extension instrumenting Vite). Try disabling Console Ninja and/or `npm run dev` in a plain terminal, then `npx playwright test guide-dictation` should stop erroring at the store lookup. Until this is fixed, NO UI change can satisfy GOAL #8 (touched-area e2e). **Then finish the PDFReader fix** (apply the epoch guard, flip fixme→test, red→green) and continue the **remaining axis-1-ish candidate `ai.js:196` double-encoded non-stream response**, then the rest of the Sync/store · Reader/multimodal · AI · Perf/UX clusters.
...
DO (per finding, TDD): reproduce it in a red test FIRST (prove it's real against live code) → if real, minimal fix → gate green, one cluster per commit; if NOT real, note "refuted — why" in the review doc and move on. Lead with the env fix + the ready PDFReader fix, then `ai.js:196`. Run the touched-area e2e before shipping any UI change (GOAL #8) — which REQUIRES the env fix above.
...
- 🟡→🔬 **PDFReader sentence doc-swap (axis-1 confident-wrong content) — VERIFIED REAL, fix ready, ship DEFERRED (2026-07-06).** In-flight sentence translations for doc A attach their English to doc B: `sentenceId` is positional (`page:para:token` → doc A & doc B share `…:0:0`), `translateDocument` resolves (not throws) with partial results on abort, and both sentence paths (`runSentenceTranslation`, `fetchSentenceEnglish`) write `setSentenceGloss` after the await with NO staleness guard (the word/OCR paths guard `signal.aborted`). A doc-A translation landing after the swap repopulates the cleared gloss, and doc B's same-position sentence reveals doc A's English. **Fix ready:** document-epoch guard — bump `docEpochRef` in `resetGloss`, capture at translation start, drop the write if the epoch changed (epoch, not content-match: doc B parses after resetGloss so a content check wrongly matches mid-parse). **⚠️ SHIP BLOCKED ON ENVIRONMENT:** the faithful test is a doc-swap-mid-flight e2e (`tests/e2e/pdf-sentence-docswap.spec.js`, deterministic gated-mock, landed as `test.fixme`) — but the **local dev/e2e env renders a BLANK app under Playwright Chromium** (`#root` stays empty; EVERY store-binding spec fails identically — `pdf-replace`, `guide-dictation`, all of them; no JS error, only a "Console Ninja connected to Vite" line → likely Console Ninja's Vite instrumentation breaking module execution in headless Chromium). Per the ship-bar I did NOT ship the fix unverified. **NEXT-SESSION TODO:** (1) fix the env — try disabling the Console Ninja VS Code extension and/or run `npm run dev` in a plain terminal, then `npx playwright test guide-dictation` should stop erroring at the store lookup; (2) apply the epoch guard; (3) flip `test.fixme`→`test`; (4) confirm red→green. This ALSO blocks GOAL #8 (run touched-area e2e before shipping UI) for the whole team until fixed — logged to GOAL backlog.
