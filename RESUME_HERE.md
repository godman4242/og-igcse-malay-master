...
- [x] **MICRO-GUIDE UDL STYLE — /study pilot shipped (attended 2026-06-24). Page-tour steps were too long (Kheshav, ADD, wants UDL). New style: one idea/step, body ≤~14 words, action-first, NO separate `example:` line, ≤5 steps. /study rewritten 6 long steps → 5 short centered cards (commit `d5e8246`, deploy READY). Spec: `docs/superpowers/specs/2026-06-24-micro-guide-udl-style.md`.**
  STILL OPEN (queued, NOT yet built — Kheshav saw "no dots/numbers still" because these aren't shipped): **GOAL #11 dots progress** (replace the visible "{{current}} of {{total}}" / "4 of 22" with dots when total≤7, a slim bar when >7; each dot a button → `onJump(i)` so jump-to-step is PRESERVED — touches `popoverDecorations.makeProgressJumpable` + `guideController.js:621` + e2e guide-drag-dock/pause-skip/pdf-reader/pdf-chaos) and **GOAL #10 rollout** (the short-step style to the other 20 routes, ONE page/commit, per the spec). The full tour (`tourSteps.js`, ~22 steps) gets the bar + brevity pass. ▶ Next session = build #11 then #10 (see the kickoff). **Recurring risk flagged:** Kheshav repeatedly sees STALE prod after a deploy (PWA service-worker auto-update not reaching him reliably) — if it affects him it affects students → investigate (GOAL backlog).
