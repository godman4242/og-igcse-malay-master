...
### → THE KICKOFF (copy everything between the `'''` lines): Malay Beginner On-Ramp Phase 1 — the curated survival-starter deck

*Superseded the "Land the Tutor Output Contract v1" kickoff on 2026-07-14 — that bet is **DONE**.*
*✅ **Tutor Output Contract v1 SHIPPED to `main`** (merge of `feat/tutor-output-contract`, 12 commits, 2026-07-14 → prod deploy). **Layer A** (the `PER-TURN DISCIPLINE` fragment in `CIKGU_SYSTEM_PROMPT` + the learner-scaffold note folded into `contextNote`) is **LIVE** for every AI-tier Cikgu turn — NOT flag-gated, so landing did change live tutor behaviour (verified in-browser: Gemini reply led with "Answer:", concise). **Layer B** (`enforceTutorTurn` length-truncation + `⟦CTRL⟧` control-strip in `src/pages/CikguBot.jsx sendMessage`) is **wired but flag-OFF** — `TUTOR_CONTRACT_ENABLED = false` in `src/lib/tutorContract.js`, an inert identity passthrough until flipped. Evidence at land: gate green (2170 unit tests), `node scripts/ai-tier-eval/leakEval.mjs` = **precision/recall 1.00 / 1.00 on a 6-item STARTER fixture** (⚠️ below the spec's ~100-item bar), browser smoke confirmed truncation-at-sentence-boundary (263→209 words) + control-strip (stream + final, so TTS-safe) + dark/light, no crashes across the Gemini/OpenRouter/Supabase branches. Expert-tier stays exempt; prompt parity guarded by `cikguSystemPrompt.test.js`. **To flip Layer B ON later (its own future bet, NOT this one):** expand `scripts/ai-tier-eval/leakFixture.mjs` to ~100 labeled items incl. hard negatives (needs Kheshav's Gemini key) → confirm leak recall + length-cap conformance ≥95% → set the flag `true` → re-smoke dark/light + voice → land. The On-Ramp bet below was this kickoff's ALTERNATIVE and is UNCHANGED.*

```
'''
Malay Beginner On-Ramp Phase 1 — design + build a curated, OPT-IN Malay survival-starter deck. The #1 beginner gap: a new Malay learner lands on ZERO cards with no "start here" (English mode already has `seedEnglishStarter`; Malay has nothing). This is a from-scratch feature — brainstorm + plan before code.

⚡ ACTIVATE FIRST: Claude Code CLI in repo `og igcse malay master` · Opus 4.8 @ xhigh, `/fast` on · start on `main` (pull first), cut a fresh branch `feat/malay-starter-deck` · run superpowers:brainstorming → writing-plans BEFORE touching code · Vercel MCP on (deploy check only) · no new MCP/plugins/installs.

WHY: The app assumes you can already read Malay. A curated ~40–50-word survival starter (greetings/pronouns/numbers/essential verbs+nouns/question words), opt-in with one tap on the Malay empty-state, gives a zero-Malay beginner a real Day-1 path into the FSRS loop. Full design + already-flagged decisions live in the "✅ NEXT BET CHOSEN (2026-07-06)" bullet further down — READ IT, don't re-derive.

READ FIRST: the "✅ NEXT BET CHOSEN (2026-07-06)" bullet below (design + decisions) · `seedEnglishStarter` in `src/store/useStore.js` (mirror its shape for `seedMalayStarter`) · `src/lib/cardLang.js` + `glossPlanFor` (Malay/English decks NEVER mix) · `src/data/dictionary.js` (entry shape `{m,e,ex,box}`). SOURCE word lists (local, OUTSIDE the repo): `~/Downloads/igcse-malay-vocabulary-malayenglish-format/*.html` (official 0546 list) + `~/Downloads/IGCSE_Malay_Complete_Guide.html` (glossed roleplays + example sentences).

DO: (1) brainstorm→plan the ~40–50-word list, each word with an example sentence — Claude VERIFIES every Malay entry against a real source before it ships (already caught pemandu="driver" mis-glossed "guide"). (2) Build `seedMalayStarter()` mirroring `seedEnglishStarter` (emits `lang:'ms'` cards via `glossPlan`). (3) Wire an OPT-IN "Start with a beginner deck" one-tap on the Malay empty-state — NEVER auto-seed (reveal-gate ethos). (4) Update README + the driver.js tour (`src/lib/guide/`) in the SAME change. (5) Gate green + a red-proofed test (seed count / lang / no cross-language mix).

DON'T BREAK: opt-in only, never auto-seed · Malay & English decks never mix (`cardsForLang`/`studyLang`) · every Malay gloss Claude-verified (confident-wrong Malay = the worst defect) · ~40 words, not 1,300 (the full list is the SOURCE, not the starter — a huge seed drowns FSRS + the beginner) · no STORE_VERSION bump unless the seed adds new persisted state.

DONE: brainstorm+plan written, ~40–50 verified starter words, `seedMalayStarter` + opt-in empty-state shipped, README + tour updated, gate green + test, Vercel READY on upg-, this kickoff superseded.
'''
```
