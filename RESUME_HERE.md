...
### → THE KICKOFF (copy everything between the `'''` lines): Crawler-facing SEO — make the app findable on Google

```
'''
Crawler-facing SEO — make the app findable on Google. Kheshav chose PUBLIC / search-discovery (2026-07-14): today Google's crawler sees a blank JS shell with ONE generic title/description across all 21 routes (the client-side `<Meta>` sets them only AFTER JS runs — crawlers often don't wait). GOAL: each route's real title/description/canonical/OG visible in the RAW HTML a crawler receives, measured against a real crawler check.

⚡ ACTIVATE FIRST: Claude Code CLI in repo `og igcse malay master` · **Opus 4.8 @ xhigh + `/fast`** (surgical architecture; Fable 5 `high` optional for the research/scoping ONLY IF you re-confirm access — past the 2026-07-12 time-box) · start on `main` (pull first) → branch `feat/crawler-seo` · **context7 MCP + WebSearch ON** (Vite / prerender docs + practitioner reviews — do NOT answer from memory) · **Vercel MCP ON** (deploy check + og- project robots/noindex) · run **superpowers:brainstorming → writing-plans BEFORE any code** · no new installs.

READ FIRST (don't re-derive the decision): `docs/loop/GOAL.md` → 🎖️ Kheshav-ranked epics #1 (approach + rationale) · the audit `~/Downloads/igcse-malay-master-audit.md` (§3.1–3.6 SEO, §7 head-management notes) · `src/components/Meta.jsx` (client-side head-setter — already supplies per-route title/desc; a prerender consumes it) · `vite.config.js` (VitePWA generateSW — the approach must NOT break the SW precache or SPA fallback) · `public/robots.txt` + `public/sitemap.xml` (STATIC, committed, SHARED by both deployments → both hardcode upg-) · `src/App.jsx` (21 routes) · `index.html` (static `<head>`).

DO (brainstorm→plan FIRST, then build):
1. **DECIDE THE APPROACH with evidence — don't presuppose prerender:** build-time prerender vs. rely on Google's own JS rendering + a clean sitemap + crawlable per-route meta vs. SSR. The audit notes Google CAN render JS (slower / less reliable). Pick the LIGHTEST approach that measurably shows per-route meta to a crawler; decide-and-flag. If prerender: research the tool for Vite 8 + React 19 on Vercel static hosting (candidates to verify current + maintained: `vite-react-ssg`, `vite-plugin-prerender`, a `@prerenderer` post-build step).
2. **Emit per-route `<head>`** (title / description / canonical self-URL / OG) into the crawler-visible HTML. ⚠️ RISK: the app is a heavy stateful SPA (Zustand+localStorage, Web Speech, transformers.js / Tesseract WASM) — if ANY eager module touches `window`/`localStorage`/`navigator` at import time, build-time prerender crashes; audit + guard the SSR-unsafe globals FIRST.
3. **Canonical = upg-, `noindex` the og- mirror** (two identical public sites = duplicate-content harm). Per-deployment `VITE_BASE_URL` (build-time) feeds robots/sitemap/canonical so the og- Vercel project builds a noindex robots — a committed robots.txt can't target only og-.
4. **Per-page H1 = the page name** (`Layout.jsx:209` currently makes "ooga da boogadamalay" the H1 on every page) — keep the gradient brand title lower, or `sr-only` a per-page H1.

DON'T BREAK: `npm run build` + the Vercel deploy pipeline (any prerender runs INSIDE the prod build — a crash blocks ALL deploys) · the VitePWA service-worker precache + SPA fallback · the client `<Meta>` (keep it — it feeds the prerender) · the gate (2174 tests, lint 0-err) · no STORE_VERSION change (no app state).

PROVE IT (measurable DONE — approach-agnostic): a real crawler check — Google Rich Results test AND/OR `curl -A Googlebot <route>` — returns each route's UNIQUE `<title>` + `<meta name=description>` + `<link rel=canonical self-URL>` + per-route OG (paste 2–3 routes' `<head>`; if the approach is prerender, also `curl` the built per-route HTML directly) · `curl` the og- deployment → `noindex` robots while upg- stays indexable · each page's H1 = its own name · gate green · Vercel READY on upg- · README/tour updated if user-facing · RESUME_HERE + GOAL.md refreshed, this kickoff superseded.
'''
```
...
- **#1 SEO + a11y hardening** (source: `~/Downloads/igcse-malay-master-audit.md`, JClaw 2026-07-14). ✅ **Quick-wins SHIPPED** (branch `feat/seo-a11y-quickwins`, 2026-07-14): `<Meta>` added to the 12 pages that lacked it (correct browser-tab titles — all 21 pages now covered) + FirstRunCard button contrast → `--color-on-bright` (CLAUDE.md label-on-fill rule; fixes the 3.21:1 fail). Content-truth caught + fixed by `examPaperLabels.test.js`: dropped "Paper 4"/"Paper 3" from the Listening/Speaking Meta descriptions (bilingual surfaces name no paper number). Gate green (2174 tests). ✅ **DECIDED 2026-07-14 — Kheshav wants PUBLIC / organic-search discovery → crawler SEO is GO** (its own research→plan→build session; touches the prod build pipeline, do NOT rush at a session tail). Approach: (a) **build-time prerender** so crawlers (no JS) see per-page title/canonical/OG — RESEARCH the right tool for Vite 8 + React 19 first (don't pick from memory), and verify crawler output for real (curl built HTML / Rich Results test), not just a green build; (b) **canonical = `upg-`, `noindex` the `og-` mirror** (two identical public sites = duplicate-content harm) via a per-deployment `VITE_BASE_URL`/robots flag — same repo, so a committed robots.txt can't target only og-; (c) **per-page H1 = the page name** (`Layout.jsx:209` currently makes the "ooga da boogadamalay" codename the H1 everywhere — a VISIBLE gradient title, NOT invisible as the audit claimed). Full detail: `docs/loop/GOAL.md` → 🎖️ Kheshav-ranked epics #1. Stale audit item: "code-split routes" — already lazy-loaded.
