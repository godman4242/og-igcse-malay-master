...
**START AT A10** (`src/components/study/FlashcardMode.jsx:179` — the flashcard's answer face sits in
the accessibility tree permanently, so a screen-reader learner hears the English gloss in the same
pass as the Malay prompt and every card is a free look, defeating the retrieval gate the whole app
is built on). **A1–A9 + B4 are SHIPPED** — see the "BATCH A PROGRESS" table below for what landed and
the evidence; do NOT re-take them. **13 Batch A rows remain (~35 h): A10–A22.**
