...
WHY: ALL 16 CONFIRMED defects (#1–#16, incl. the P0 Malay grader and the #15 share-deck build) are SHIPPED gate-green (see the progress block below). What's left from the review is the 🟡 PLAUSIBLE list — findings extracted but NOT yet independently verified. The review's own instruction: "do not fix blind; verify first." **4 PLAUSIBLE items already cleared** (all VERIFIED REAL + FIXED): QuickReview studyLang leak · cloud `card_key` lang collision · `writingGrader` "sehinggakan" reward/flag contradiction · `pronunciation` positional cascade. **Remaining axis-1-ish candidates worth catching next:** `useStore` `promoteMistakeToCard` cross-language card link + `:1627` dedupe-bump keeping `reviewed:true` (data correctness); `PDFReader` in-flight sentence translation attaching doc A's English to doc B (content correctness); `ai.js:196` double-encoded non-stream response. Then the rest of the Sync/store · Reader/multimodal · AI · Perf/UX clusters.
...
DO (per finding, TDD): reproduce it in a red test FIRST (prove it's real against live code) → if real, minimal fix → gate green, one cluster per commit; if NOT real, note "refuted — why" in the review doc and move on. Lead with the remaining axis-1-ish items listed in WHY (mistake/dedupe data correctness, then the reader translation doc-swap). Run the touched-area e2e before shipping any UI change (GOAL #8).
