...
**START AT A12** (`src/store/useStore.js:1589` — consuming a streak freeze on a milestone day instantly
refunds it and re-fires the milestone celebration, so the streak becomes unloseable and the whole
loss-aversion mechanic stops meaning anything). **A1–A11 + B4 are SHIPPED** — see the "BATCH A PROGRESS"
table below for what landed and the evidence; do NOT re-take them. **11 Batch A rows remain (~32 h):
A12–A22.**
