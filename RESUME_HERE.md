...
### → THE KICKOFF (copy everything between the `'''` lines): Malay Beginner On-Ramp — Phase 1 (brainstorm → spec → build the survival-starter deck)

```
'''
Design AND build Phase 1 of the "Malay Beginner On-Ramp": a curated, opt-in Malay survival-starter deck a from-zero learner can seed in one tap. Brainstorm → short spec → build the MVP → gate green, all this session (Phase 1 is small).

⚡ ACTIVATE FIRST: Claude Code CLI in repo `og igcse malay master` · **Opus 4.8 @ xhigh, /fast on** · **skill: `superpowers:brainstorming`** (open with it — this is creative/product work, not a bugfix) then `superpowers:writing-plans` for the short spec · no MCP/plugins/installs needed.

WHY: The whole 2026-07-03 adversarial review is now CLEARED — all 16 CONFIRMED defects shipped, and every 🟡 PLAUSIBLE finding is verified+fixed except 3 documented-and-deferred design-calls (see below). The remaining product gap is the #1 beginner problem: a new Malay (0546) learner lands on ZERO cards with no "start here" (English mode already has `seedEnglishStarter` at `useStore.js` ~:1315; Malay beginners don't). Kheshav pre-chose this bet 2026-07-06 over the concierge/ergonomics epics. This is personalization-aspect work — the top aspect in the order personalization→learning-science→code.

READ FIRST: (1) RESUME_HERE "✅ NEXT BET CHOSEN (2026-07-06)" bullet — the approved-in-principle Phase-1 design (seedMalayStarter mirroring seedEnglishStarter; ~40–50 highest-value words; opt-in empty-state button; I verify every Malay entry). (2) `useStore.js` `seedEnglishStarter` — the pattern to mirror (action shape, card schema `{m,e,t,p,ex,mn}`, STORE_VERSION handling). (3) The empty-state / Import surface where the one-tap "Start with a beginner deck" button lands. SOURCE files (local, OUTSIDE repo — confirm present, else ask): `~/Downloads/igcse-malay-vocabulary-malayenglish-format/*.html` (official 0546 list ~1,300 pairs) + `~/Downloads/IGCSE_Malay_Complete_Guide.html` (glossed roleplays + example sentences).

DO: brainstorm the word list + surfacing UX first (decide-and-flag the forks) → tiny spec in `docs/superpowers/specs/` → build `seedMalayStarter()` (~40–50 words: greetings/pronouns/numbers/essential verbs+nouns/question words, each with an example sentence) → wire the OPT-IN one-tap button on the empty-state (default deck is `cards: []`) → red→green test that seeding adds the deck once + is idempotent + respects `studyLang` scoping. **I (Claude) verify EVERY Malay word+gloss before ship — Kheshav is not Malay-fluent and I'm the quality gate (already caught `pemandu`="driver" mis-glossed as "guide"). Ground each against a real source, not memory.**

DON'T BREAK: opt-in, NEVER auto-seed (reveal-gate ethos — a beginner choosing to start ≠ forced content). Malay/English decks never mix in a session (`cardsForLang`/`studyLang`). If the store shape changes, bump STORE_VERSION + add a migration. ~40 words, NOT 1,300 (a big seed overwhelms FSRS + the learner; the full list is the SOURCE, not the starter). Update README + the driver.js tour (`src/lib/guide/tourSteps.js`) in the SAME commit.

DONE: gate green (build+test+lint), a red→green seedMalayStarter test, README + tour updated, RESUME_HERE progress note + a fresh next kickoff. Confirm Vercel READY on upg- after the main push.

ALTERNATIVE (clean build-safe, no design needed — pick if you'd rather keep hardening): finish the review TAIL — (a) the last Reader item, `translatePage`/`cancelTranslate` cancel-retranslate race (1-line abort-ref guard; write a gated-mock e2e à la `pdf-sentence-docswap` to red-prove it first); (b) run the 5 never-swept adversarial finder scopes (bilingual-v34, study-modes, app-shell-guide, wildcard-integration, docs-drift) — could surface new axis-1 bugs. The 2 DEFERRED `syncEngine` items (add/remove reorder + dead-letter of deletions) need YOUR queue-semantics design call before any patch — not loop-safe.
'''
```
