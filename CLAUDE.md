...
- **Card-creation gloss direction follows `studyLang`** via `glossPlanFor(studyLang)` (`src/lib/glossPlan.js`, pure) → `{ lang, from, to, useStemmer }`. The active `studyLang` is the **source-language signal** (one place, so Import + the reader can't diverge — mirrors `localeFor`). **Import.jsx (F5 Increment 1, shipped)** threads it: `studyLang='en'` → English→Malay gloss via the lazy `dictionaryEn` seed + `translateWord(w,'en','ms')`, no Malay stemmer, card `lang:'en'`; `'ms'` is byte-identical to before. **PDFReader.jsx still stamps `lang:'ms'`** (English-source reader path = F5 Increment 2, pending — must thread `glossPlanFor` **without touching the reveal-gated `{pages}` core, N1**). **Still Phase 1.5+ (not built):** bilingual surfaces following `studyLang`, English mistake→FSRS promotion, productive (gloss→word) direction. Spec/plan: `docs/superpowers/{specs,plans}/2026-06-14-true-english-study-mode*`.
