...
**Goal:** Add evidence-based learning science features (elaborative feedback, interleaved practice, metacognitive confidence, mistake clustering) to IGCSE Malay Master, then wire up 4-tier access control with anonymous telemetry via Supabase.
...
**Goal:** When a student gets a grammar drill or vocab card wrong, show them *why* with the underlying rule, a mnemonic, and related examples.
...
export const GRAMMAR_FEEDBACK = {
  ...
  'me- + r': {
    explanation: 'When meN- meets R, use "me-" (no nasal). R is a liquid consonant.',
  ...
  'me- + ny (already nasal)': {
    explanation: 'NY is already a nasal sound, so just use "me-" without adding another nasal.',
  ...
  'mem- + f': {
    explanation: 'When meN- meets F, use "mem-" and keep the F. F is a labiodental like P but doesn\'t drop.',
  ...
  'ber- + main': {
    ...
    mnemonic: 'ber- is simple — it rarely changes. Watch for the 2 exceptions: belajar and bekerja.',
    examples: [
  ...
  'ber- + cakap': {
    ...
    relatedRule: 'ber- is consistent — only 2 irregular forms (belajar, bekerja).',
  },
  'ber- + sedia': {
    explanation: 'Standard ber- prefix application.',
    mnemonic: 'ber- + consonant = no change.',
  ...
  'ber- + asa → berasa': {
    explanation: 'Standard ber- prefix with vowel-initial root. Unlike belajar, this keeps the R.',
  ...
  'pen- + t → t drops (doer noun)': {
    explanation: 'peN- follows the SAME rules as meN-. T drops with pen-, becoming a doer/agent noun.',
    ...
    relatedRule: 'meN- = verb (menulis = to write), peN- = noun (penulis = writer).',
  },
  'pen- + d (doer noun)': {
  ...
  'pe- + kerja (doer noun)': {
    explanation: 'pe- is used for K-initial roots in peN- (K doesn\'t drop in this case for the doer meaning).',
    mnemonic: 'pekerja (worker) uses simple pe-, not peng-.',
    examples: [
      { root: 'kerja', result: 'pekerja', gloss: 'worker' },
    ],
    relatedRule: 'Compare: some K-initial roots use peng- (pengarang), some use pe- (pekerja).',
  },
  'pem- + b (doer noun)': {
  ...
  'Object becomes subject': {
    explanation: 'In passive voice, the object moves to subject position. The agent follows "oleh".',
    mnemonic: 'Flip the sentence: Object first, then di-verb oleh Subject.',
    examples: [
      { root: 'Ali membaca buku itu.', result: 'Buku itu dibaca oleh Ali.', gloss: 'That book is read by Ali.' },
    ],
  ...
  'S drops in meN- but returns in di-': {
    ...
    examples: [
      { root: 'Emak menyediakan sarapan.', result: 'Sarapan disediakan oleh emak.', gloss: 'Breakfast is prepared by mother.' },
    ],
    relatedRule: 'All four (P, T, K, S) return in passive: memotong → dipotong, etc.',
  },
...
import { CheckCircle, XCircle, ArrowRight, Trophy, BookOpen, Zap, PenLine, Volume2 } from 'lucide-react'
    ...
    <div className="space-y-3 animate-fadeUp">
      ...
      {current.type === 'vocab' && (
        <div className="rounded-2xl p-5" style={{ background: 'var(--color-card)', border: '1px solid var(--color-border)' }}>
          ...
          {!flipped && !feedback && (
            <button onClick={() => setFlipped(true)} className="w-full p-3 rounded-xl font-bold text-sm"
...
```js
        ...
        if (version < 6) {
          state = {
            ...
            interleaveSettings: state.interleaveSettings || { enabled: true, vocabRatio: 0.5, grammarRatio: 0.3, comprehensionRatio: 0.2 },
...
export function trackEvent(event, payload = {}) {
  ...
  if (cloudEnabled) {
    try { sendTelemetryEvent(event, payload) } catch {}
  }
}
