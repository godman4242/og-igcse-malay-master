...
2. Confirm the 4 open decisions in spec §7 with Kheshav (defaults are sensible;
   one quick AskUserQuestion). Don't build past them blind.
...
### Step 4 — eyeball + e2e
- `tests/e2e/saved-word-tap.spec.js` per spec §8 (5 tests). Reuse `bindStore` +
  the inject-prose pattern from `saved-word-highlight.spec.js`. CRITICAL tests:
  selection-not-broken (press-drag → no popover) and link-safety.
