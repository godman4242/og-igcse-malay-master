...
**#2 — Malay On-Ramp Phase 2 → REFRAMED as "dictionary example sentences" (IN PROGRESS 2026-07-15).** The original framing ("fold the full official-0546 list into `dictionary.js` + expand `TOPIC_PACKS` to A–E buckets") is DEAD on grounding: the ~1,300-word source folder no longer exists, and `dictionary.js` (825 clean entries) is already the largest, cleanest asset — coverage isn't the gap. The real gap is DEPTH: `dictionary.js` has no example sentences, so cloze/Produce can't work for words that lack one. Reframed bet = give every dictionary word a verified example (grow the already-shipped parallel map `src/data/dictionaryExamples.js`), additive + batched. **Machinery + Batches 1–9 shipped** (merged to main + LIVE; example-quality content-lint guard; the 3 card-creation paths use `getExample`; coverage **254 → 659/825**). Remaining = the ~45-word/commit batch grind to 825 (Claude = the Malay gate, web-verify every example — Batch 7 caught **8** defects /45 via 3 parallel PRPM/Kamus-Dewan verification subagents, the highest rate yet: the meN- verbs cluster alphabetically and each hides a transitivity or preposition trap, e.g. `membazir` (intransitive) vs `membazirkan` (transitive)).
...
0-bis. **Three `dictionary.js` gloss/headword items DEFERRED out of example-Batch 8 (2026-07-29) — each needs a
   ...
   - *(Separate nit, found by the Batch-8 self-review sweep, PRE-EXISTING from the original 254-word seed
     ...
     ("Langit malam dipenuhi bintang yang berkelipan.") — still the only duplicate value in all 659 entries
