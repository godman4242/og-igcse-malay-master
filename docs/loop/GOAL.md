...
**#2 — Malay On-Ramp Phase 2 → REFRAMED as "dictionary example sentences" (IN PROGRESS 2026-07-15).** The original framing ("fold the full official-0546 list into `dictionary.js` + expand `TOPIC_PACKS` to A–E buckets") is DEAD on grounding: the ~1,300-word source folder no longer exists, and `dictionary.js` (825 clean entries) is already the largest, cleanest asset — coverage isn't the gap. The real gap is DEPTH: `dictionary.js` has no example sentences, so cloze/Produce can't work for words that lack one. Reframed bet = give every dictionary word a verified example (grow the already-shipped parallel map `src/data/dictionaryExamples.js`), additive + batched. **Machinery + Batches 1–7 shipped** (merged to main + LIVE; example-quality content-lint guard; the 3 card-creation paths use `getExample`; coverage **254 → 569/825**). Remaining = the ~45-word/commit batch grind to 825 (Claude = the Malay gate, web-verify every example — Batch 7 caught **8** defects /45 via 3 parallel PRPM/Kamus-Dewan verification subagents, the highest rate yet: the meN- verbs cluster alphabetically and each hides a transitivity or preposition trap, e.g. `membazir` (intransitive) vs `membazirkan` (transitive)).

  ↳ **Two `dictionary.js` data findings surfaced by Batch 7 verification — NOT acted on, because `dictionary.js` is this bet's hard never-edit invariant.** Both are real content defects worth a separate, deliberate decision: (a) **`mee` is not in Kamus Dewan at all** — DBP's baku spelling is **`mi`**, and JendelaDBP's own column ("Ejaan mi, mee, atau mie?") rules *mee*/*mie* incorrect, so the app currently teaches a non-standard spelling; (b) the **`masak`** gloss reads "cook", but bare `masak` is stative in Kamus Dewan (*cooked/ripe*) — the transitive act is `memasak`, which is already its own entry, so the gloss should read "cooked/ripe" or the two entries collapse in the learner's head. *Done:* decide whether to correct headword spellings/glosses in `dictionary.js` (needs a migration thought — existing user cards carry the old `m` string, so a headword rename is NOT purely additive; a gloss-only fix IS). Spec `docs/superpowers/specs/2026-07-15-dictionary-examples-design.md` · Plan `docs/superpowers/plans/2026-07-15-dictionary-examples.md` (Task 3 = batch template). (The `TOPIC_PACKS`→A–E-buckets restructure is dropped from this bet — reconsider separately if wanted.)
