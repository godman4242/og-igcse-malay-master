...
**#2 — Malay On-Ramp Phase 2 → REFRAMED as "dictionary example sentences" (IN PROGRESS 2026-07-15).** The original framing ("fold the full official-0546 list into `dictionary.js` + expand `TOPIC_PACKS` to A–E buckets") is DEAD on grounding: the ~1,300-word source folder no longer exists, and `dictionary.js` (825 clean entries) is already the largest, cleanest asset — coverage isn't the gap. The real gap is DEPTH: `dictionary.js` has no example sentences, so cloze/Produce can't work for words that lack one. Reframed bet = give every dictionary word a verified example (grow the already-shipped parallel map `src/data/dictionaryExamples.js`), additive + batched. **Machinery + Batches 1–4 shipped** (merged to main + LIVE; example-quality content-lint guard; the 3 card-creation paths use `getExample`; coverage **254 → 434/825**). Remaining = the ~45-word/commit batch grind to 825 (Claude = the Malay gate, web-verify every example — Batch 4 caught 2/45 defects via 3 parallel PRPM/Kamus-Dewan verification subagents). Spec `docs/superpowers/specs/2026-07-15-dictionary-examples-design.md` · Plan `docs/superpowers/plans/2026-07-15-dictionary-examples.md` (Task 3 = batch template). (The `TOPIC_PACKS`→A–E-buckets restructure is dropped from this bet — reconsider separately if wanted.)
