// Phase 3c T16 — the Listening picker deep-dive Full Page Guide. The header
// ▶ "Tour this page" now works on /listening: a centered intro, then an
// arrow pointing at each picker control (the passage list + its badge row),
// then a centered summary of the hear-it loop (Play / replay / unlock-questions
// / MCQ / score / transcript). Those reading-screen controls only mount after a
// passage opens, so they are taught without an arrow.
//
// Like /comprehension, Listening is a normal (non-theater) page, so the
// deep-dive entry is the standard HEADER ▶, and the picker anchors are mounted
// at landing (the passage list is static + never empty, so the first card and
// its badge row always render).
//
// Two proofs:
...
//
// Run solo:
//   npx playwright test guide-listening --config tests/e2e/playwright.config.js
...
test('deep dive: ▶ on Listening launches and the first arrow draws', async ({ page }) => {
  await prep(page)
  await page.goto('/listening', { waitUntil: 'networkidle' })

  // Picker landing — the passage list is up (Listening never enters theater mode).
  await expect(page.locator('[data-guide="listening-passages"]')).toBeVisible()
  ...
  await expect(popover).toContainText(/listening practice/i) // the centered intro step
  ...
  await expect(popover).toContainText(/Pick something to listen/i)
})
